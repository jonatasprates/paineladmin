...
abstract class banco
{
    ...
    public function selecionaCampos($objeto)
    {
        ...
        return $this->executaSQL($sql);
    }// metodo seleciona os campos
    
    public function executaSQL($sql=NULL)
    {        
        if($sql != NULL):
            $query = mysql_query($sql) or $this->trataerro(__FILE__,__FUNCTION__);
            $this->linhasafetas = mysql_affected_rows($this->conexao);
            if(substr(trim(strtolower($sql)),0,6)=='select'):
                $this->dataset = $query;
                return $query;
            else:
                return $this->linhasafetas;
            endif;
                
        else:
            $this->trataerro(__FILE__,__FUNCTION__,NULL,'Comando não encontrado na rotina',FALSE);
        endif;
    }// metodo executarSQL
    ...
    public function retornaDados($tipo=NULL)
    {
        switch (strtolower($tipo)):
            ...
            default :
                ...
                break;
        endswitch;
    }// metodo que retorna dados do banco
